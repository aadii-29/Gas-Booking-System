...
const ApplyDeliveryStaffForm = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      const allowedTypes = {
        AadharDocument: ['application/pdf', 'image/jpeg', 'image/png'],
        StaffPhoto: ['image/jpeg', 'image/png'],
        StaffSignature: ['image/jpeg', 'image/png'],
      };
      if (!allowedTypes[name].includes(file.type)) {
        toast.error(`Invalid file type for ${name}. Allowed types: ${allowedTypes[name].join(', ')}`);
        return;
      }
    }
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      formData.append(
        "StaffAddress",
        JSON.stringify({
          FlatNo: values.StaffAddress.FlatNo,
          Building_Society_Name: values.StaffAddress.Building_Society_Name.trim(),
          Area: values.StaffAddress.Area.trim(),
          City: values.StaffAddress.City.trim(),
          State: values.StaffAddress.State.trim(),
          Pincode: values.StaffAddress.Pincode,
        })
      );
      formData.append("AadharNumber", values.AadharNumber);
      formData.append("Salary", values.Salary);
      const assignedAreas = values.AssignedArea
        ? values.AssignedArea
            .split(',')
            .map((item) => item.trim())
            .filter((item) => item)
        : [];
      formData.append("AssignedArea", JSON.stringify(assignedAreas));
      ...
      setSubmittedValues({
        ...values,
        AssignedArea: assignedAreas,
      });
      
      // Combine backend response with submitted values
      const details = {
        ApplicationID: response.deliveryStaff?.ApplicationID || 'N/A',
        AgencyID: response.deliveryStaff?.AgencyID || values.AgencyID,
        StaffName: response.deliveryStaff?.StaffName || values.StaffName,
        Approval_Status: response.deliveryStaff?.Approval_Status || 'Pending',
        Documents: response.uploadedDocuments || {},
        DOB: values.DOB,
        StaffMobileNo: values.StaffMobileNo,
      ...
      if (!response.deliveryStaff?.ApplicationID) {
        console.warn("API response missing expected deliveryStaff data:", response);
        toast.warn("Application submitted, but some details are missing. Please check application status.");
      }
        ...
        {showSuccess && applicationDetails && (
          <div className="mb-4 bg-green-50 border border-green-200 text-gray-900 px-4 py-3 rounded">
            ...
            <div className="mt-4">
              ...
              <p className="text-sm">
                <strong>Mobile Number:</strong> {applicationDetails.StaffMobileNo}
              </p>
              <p className="text-sm">
                <strong>Email:</strong> {applicationDetails.StaffEmail}
              </p>
              <p className="text-sm">
                <strong>Aadhar Number:</strong> {applicationDetails.AadharNumber}
              </p>
              <p className="text-sm">
                <strong>Expected Salary:</strong> {applicationDetails.Salary}
              </p>
              <p className="text-sm">
                <strong>Assigned Area:</strong>{" "}
                {applicationDetails.AssignedArea?.length > 0
                  ? applicationDetails.AssignedArea.join(", ")
                  : "N/A"}
              </p>
              <p className="text-sm">
            ...
            {applicationDetails.Documents && (
              <div className="mt-4">
                ...
                <ul className="list-disc list-inside text-sm">
                  <li>
                    Aadhar Document:{" "}
                    {applicationDetails.Documents.AadharDocument
                      ? applicationDetails.Documents.AadharDocument.split("/").pop()
                      : "Not provided"}
                  </li>
                  <li>
                    Staff Photo:{" "}
                    {applicationDetails.Documents.StaffPhoto
                      ? applicationDetails.Documents.StaffPhoto.split("/").pop()
                      : "Not provided"}
                  </li>
                  <li>
                    Staff Signature:{" "}
                    {applicationDetails.Documents.StaffSignature
                        ...
                        <div className="col-span-full">
                          ...
                          <div className="mt-0.5">
                            <Field
                              type="text"
                              name="AssignedArea"
                              placeholder="Enter areas (e.g., Vimannagar,Kalyaninagar)"
                              className="block w-full rounded-md bg-white px-3 py-2 text-base text-gray-900 border border-gray-300 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 sm:text-sm"
                            />
                            <ErrorMessage
                              name="AssignedArea"
